...
for(const [name,width,height] of [['desktop',1440,1000],['mobile',390,844],['small',320,800]]){
 ...
 assert.equal(await page.locator('.portfolio-card:visible').count(),total);
 await page.getByRole('button',{name:'Sound & music',exact:true}).click();assert.equal(await page.locator('.portfolio-card:visible').count(),await page.locator('.portfolio-card[data-category="Sound & music"]').count());
...
const page=await browser.newPage({javaScriptEnabled:false});await page.goto(base);assert.equal(await page.locator('.portfolio-card:visible').count(),total);await browser.close();
